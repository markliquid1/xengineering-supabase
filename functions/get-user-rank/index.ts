  ...
  try {
    ...
    if (type === 'energy') {
      ...
      if (stats && stats.length > 0) {
        ...
        const { data: profiles } = await supabase.from('user_profiles').select('device_uid, username, boat_type, boat_length_ft').in('device_uid', deviceUids).eq('profile_public', true);
        stats.forEach((entry)=>{
      ...
      const { data: profiles } = await supabase.from('user_profiles').select('device_uid, username, boat_type, boat_length_ft').eq('boat_type', boat_type).gte('boat_length_ft', min_length || 0).lte('boat_length_ft', max_length || 999).eq('profile_public', true);
